...
func TestScalar(t *testing.T) {
	...
	t.Run("scalar_number_inf_neg", func(t *testing.T) {
		vec = assertParse(t, vec, nil, 0)
		assertType(t, vec, "", vector.TypeNumber)
		assertNumber(t, vec, "", math.Inf(-1))
	})
	...
	t.Run("scalar_number_nan", func(t *testing.T) {
		vec = assertParse(t, vec, nil, 0)
		assertType(t, vec, "", vector.TypeNumber)
		assertNumber(t, vec, "", math.NaN())
	})
	t.Run("scalar_number_NaN", func(t *testing.T) {
		vec = assertParse(t, vec, nil, 0)
		assertType(t, vec, "", vector.TypeNumber)
...
func BenchmarkScalar(b *testing.B) {
	...
	b.Run("scalar_number_neg", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_float", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_exponential", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_exponential_neg", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_fake", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeString) }) })
	b.Run("scalar_number_inf", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_Inf", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_INF", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_inf_neg", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_Inf_neg", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_INF_neg", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_nan", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_NaN", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_number_NAN", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeNumber) }) })
	b.Run("scalar_string", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeString) }) })
	b.Run("scalar_string_folded_block", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeString) }) })
	b.Run("scalar_string_literal_block", func(b *testing.B) { bench(b, func(vec *Vector) { assertType(b, vec, "", vector.TypeString) }) })
